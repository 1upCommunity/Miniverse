class OtherPlayerDrawer {
  ...
  draw() {
    ...
    for (const i in this.all_players) {
      // if the player is close enough to draw
      if (
        screens[2].substate == "play" &&
        dist(
          this.all_players[i].position[0],
          this.all_players[i].position[1],
          camera.x,
          camera.y
        ) <
          500 + windowHeight / 2
        ...
        try {
          ...
          textSize(16);
          text(
            this.all_players[i].name,
            this.all_players[i].position[0] -
              textWidth(this.all_players[i].name) / 2,
            this.all_players[i].position[1] - 25
          );
          ellipse(
            this.all_players[i].position[0],
            this.all_players[i].position[1],
