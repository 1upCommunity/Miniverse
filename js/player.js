class Player {
  ...
  update() {
    ...
    if (this.infect_informed[1] != this.infected) {
      this.infect_informed[0] = false;
    }
    // toast if infected
  ...
  syncName(name) {
    ...
    this.db
      .ref("players/" + this.name)
      .update({ position: [this.x, this.y], name: this.name });
  }
}
